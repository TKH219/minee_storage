-- Money and quantities cross the API as decimal strings, never JSON numbers.
-- jsonb_build_object on a numeric emits a JSON number, which a JavaScript or
-- Dart client parses as a double -- exactly the float drift numeric(18,2) and
-- numeric(14,3) exist to prevent. Every such column is cast to text here.
...
as $$
  select to_jsonb(t)
    || jsonb_build_object(
         ...
         'discount_total',     t.discount_total::text,
         'buyer_charge_total', t.buyer_charge_total::text,
         'seller_cost_total',  t.seller_cost_total::text,
         'pass_through_total', t.pass_through_total::text,
         'buyer_total',        t.buyer_total::text,
         'net_revenue',        t.net_revenue::text,
         'cogs',               t.cogs::text,
         'gross_profit',       t.gross_profit::text,
         'net_profit',         t.net_profit::text,
         'net_margin',         t.net_margin::text)
    || jsonb_build_object(
         'lines', coalesce((
           select jsonb_agg(
                    to_jsonb(l) || jsonb_build_object(
                      'quantity_delta',     l.quantity_delta::text,
                      'unit_price',         l.unit_price::text,
                      'unit_cost_snapshot', l.unit_cost_snapshot::text,
                      'line_gross',         l.line_gross::text,
                      'line_cost',          l.line_cost::text)
         ...
         'fees', coalesce((
           select jsonb_agg(
                    to_jsonb(f) || jsonb_build_object(
                      'value',           f.value::text,
                      'computed_amount', f.computed_amount::text)
